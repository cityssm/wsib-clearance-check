...
describe("getClearanceByAccountNumber(validAccountNumber)", () => {
  ...
  before(async() => {
    ...
    try {
      certificate = await wsib.getClearanceByAccountNumber(accountNumber) as wsibTypes.WSIBClearance_Success;
...
describe("getClearanceByAccountNumber(invalidAccountNumber)", async () => {
  ...
  before(async() => {
    ...
    try {
      certificate = await wsib.getClearanceByAccountNumber("1") as wsibTypes.WSIBClearance_Failure;
      console.log(certificate);
